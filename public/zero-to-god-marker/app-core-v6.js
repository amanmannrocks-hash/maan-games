...
'use strict';
const N=50,MAX=4,KEY='z2g-crop-marker-v7',OLD_KEYS=['z2g-crop-marker-v6','z2g-crop-marker-v5'],DB='z2g-crop-marker-handles-v5',STORE='handles';
...
function blankLevel(i){return{level:i+1,image:`${i+1}.png`,imageWidth:1024,imageHeight:1536,sourceImage:null,done:false,savedAt:null,objects:[]}}
function fresh(){return{version:7,level:0,mode:'ellipse',levels:Array.from({length:N},(_,i)=>blankLevel(i))}}
...
function migrate(x){
 const out=fresh(),old=x&&Array.isArray(x.levels)?x:null;
 if(old){
  out.level=clamp(Number(old.level)||0,0,N-1);
  out.mode=(old.mode==='square'||old.mode==='rectangle')?'rectangle':'ellipse';
  for(let i=0;i<Math.min(N,old.levels.length);i++){
   const v=old.levels[i]||{};
   out.levels[i]={...blankLevel(i),...v,level:i+1,image:v.image||`${i+1}.png`,imageWidth:v.imageWidth||1024,imageHeight:v.imageHeight||1536,done:!!v.done,savedAt:v.savedAt||null,objects:(v.objects||[]).map(migrateObject).filter(Boolean)};
  }
 }
 out.version=7;
 return out;
}
function load(){try{let raw=localStorage.getItem(KEY);if(!raw)for(const k of OLD_KEYS){raw=localStorage.getItem(k);if(raw)break}return raw?migrate(JSON.parse(raw)):fresh()}catch{return fresh()}}
function saveLocal(){state.level=level;state.mode=mode;state.version=7;localStorage.setItem(KEY,JSON.stringify(state))}
...
async function chooseFolder(){if(!window.showDirectoryPicker){toast('Folder writing is unavailable here. Use “Select 50 Images” instead.');return}try{const h=await window.showDirectoryPicker({mode:'readwrite'});await connectFolder(h,true);toast('Original images connected.')}catch(err){if(err?.name!=='AbortError')toast(err.message||'Could not open that folder.')}}
...
function imageNumber(name){const stem=String(name).replace(/\.[^.]+$/,'');let m=stem.match(/^0*(\d{1,2})$/);if(!m)m=stem.match(/^level[-_\s]*0*(\d{1,2})$/i);if(!m)m=stem.match(/(?:^|\D)(50|[1-4]\d|[1-9])(?:\D|$)/);const n=m?Number(m[1]):NaN;return n>=1&&n<=N?n:NaN}
async function chooseImageFiles(files){const a=[...files].filter(f=>f.type.startsWith('image/')).sort((x,y)=>imageNumber(x.name)-imageNumber(y.name));const nums=a.map(f=>imageNumber(f.name));if(a.length!==N||nums.some(Number.isNaN)||new Set(nums).size!==N||Math.min(...nums)!==1||Math.max(...nums)!==N){toast('Select exactly 50 images numbered 1 through 50.');return}fileMap=new Map(a.map(f=>[imageNumber(f.name),f]));sourceMode='files';projectHandle=null;outputHandle=null;E.sourceState.textContent='Mobile/file mode: each completed level downloads as a ZIP';E.sourceState.className='source-state ready';await show(level);toast('50 original images loaded.')}
